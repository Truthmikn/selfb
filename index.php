<?php
/*
⭐️ سورس ای ار | منبع فروش سورس ربات و سایت و قالب و افزونه
...
if ($text == 'موشک' or $text=='حمله') {
  ...
  yield $MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "🚀 🛸"]);
  yield $MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "🚀🛸"]);
  yield $MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "💥 Boom Alian Be F**k Raf, Bye 😂💥  @surce_ir "]);
}
...
if ($text == 'بادکنک' or $text == 'کاکتوس') {
  ...
  yield $MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "🌵 🎈"]);
  yield $MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "🌵🎈"]);
  yield $MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "💥😐 Bomm, Ballon exploded 😥💥  @surce_ir  "]);
}
...
if($text=='گوههه' or $text=='گوه بخوررر'){
	...
	yield $MadelineProto->messages->editMessage(['peer' => $peer, 'id' => $msg_id , 'message' => 'GOH BOKHOR💩 @surce_ir ']);
}
...
if($text=='مجسمه' or $text=='حکاکی'){
  ...
  yield $MadelineProto->messages->editMessage(['peer' => $peer, 'id' => $msg_id, 'message' => '⛏🗿']);
  yield $MadelineProto->sleep(1); 
  yield $MadelineProto->messages->editMessage(['peer' => $peer, 'id' => $msg_id, 'message' => '🗿 @surce_ir ']);
}
...
if ($text == '/runlist' or $text == 'لیست فعال') {
  ...
  if($text == '/creator' or $text == 'سازنده'){
       yield $this->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "𝐂𝐫𝐞𝐚𝐭𝐞𝐝 𝖡𝗒 🤴🏻 @surce_ir_ir 🤴🏻"]);
       }
